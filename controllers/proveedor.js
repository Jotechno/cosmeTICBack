...
const proveedorPut = async (req, res = response) => {
    const { nit_cedula, nombre_proveedor, correo, direccion, telefono, estado_proveedor, observacion } = req.body//modificar

    let mensaje = ""

    try {
        const proveedor = await Proveedor.findOneAndUpdate({ nit_cedula: nit_cedula }, { nombre_proveedor: nombre_proveedor, correo: correo, direccion: direccion, telefono: telefono, estado_proveedor: estado_proveedor, observacion: observacion })//Primera llave es el nombre del atributo, el segundo es el nuevo atributo
